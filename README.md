...
Cross-repo `Novolis.*` restore uses `GITHUB_TOKEN` (`packages: read` on PR jobs, `packages: write` on merge). Pass `packages_token` to `dotnet-build` only when a PAT with `read:packages` is required.
